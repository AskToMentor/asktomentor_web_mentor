    ...
    <>
      <div className="mt-5 mb-10">
        <div className="w-full flex flex-col md:flex-row gap-5 mt-10">
          <div className="category-list-slider flex flex-row mt-4"> {/* Slider container */}
            {categories.map((category, index) => (
              <div
                key={index}
                className={`category-item ${index === 0 ? 'all-category' : ''}`}
                style={{
                  backgroundColor: index === 0 ? category.backgroundColor : 'transparent',
                  color: category.textColor || '#a1aebeff'
                }}
              >
                {category.icon && <img src={category.icon} className="icon" alt={category.name} />}
